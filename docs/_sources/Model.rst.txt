...
.. content-tabs::
    ...
    .. tab-container:: tab1
        ...
        :math:`\frac{dI^{C2}_i}{dt} =  (1-\rho'_i) \gamma_{C} I_i^{SC1}  -\gamma_{R,C} I_i^{C2}`
    ...
    .. tab-container:: tab2
        ...
        The parameters of the considered model are following:
                ...
                - :math:`\kappa = 1/d_L` transition rate of exposed individual becoming infectious, with :math:`d_L` the average number of days in this latent state
                - :math:`\gamma_{C} = 1/d_C` transition rate of going from from :math:`I^{SC1}` to :math:`I^C`, with :math:`d_C` the average number of days it takes to undergo this transition
                - :math:`\gamma_{R} = 1/d_R` recovery rate from :math:`I^{SC2}`, with :math:`d_R` the average number of days it takes to recover
                - :math:`\gamma_{R,C} = 1/d_{R,C}` recovery rate from :math:`I^{C2}`, with :math:`d_{R,C}` the average number of days it takes to recover
                - :math:`\nu = 1/d_D` death rate from :math:`I^{C1}`, with :math:`d_D` the average number of days before death occurs after being diagnosed (which mostly corresponds to reaching the hospital)
                ...
                Other parameters are needed to connect the Google mobility data the reductions in social contacts :math:`\alpha`'s.

.. They are detailed in TODO: PUT HERE LINK TO DETAILED DRAFT!
